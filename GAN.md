...
Die prominenteste Verwendung für GANs ist wohl im Bereich der Bildgenerierung und hier insbesondere der Generierung von Gesichtern nicht existierender Personen angesiedelt.
In diesem Bereich nehmen die sogenannten StyleGANs eine Vorreiterrolle ein. Auf die wesentlichen Merkmale dieser GANs gehen wir später [hier](user-content-stylegan) ein.

## Wie funktionieren GANs?
Im diesem Blog sollen GANs vorgestellt werden, welche zum Beispiel in der Lage sind, echt aussehende Bilder von Früchten zu generieren.
...
### Erste Phase
Der vordere Teil des GANs, der Generator, wird mit einer zufälligen Eingabe gefüttert. In vielen Fällen handelt es sich dabei um gaußsches Rauschen, aber auch andere Verteilungen sind möglich.
Den Input bearbeitet der Generator nun bestenfalls so, dass echt aussehende Bilder von Früchten entstehen. In den ersten Durchläufen wird dies nicht gelingen, die Ausgabe wird mehr oder weniger zufällig sein.
Dies liegt daran, dass die Gewichte des neuronalen Netzwerkes zufällig initialisiert wurden und auch noch nicht ausreichend in eine Richtung angepasst werden konnten, die es ermöglicht, echt aussehende Daten zu generieren.
Bevor die Gewichte des Generators überhaupt angepasst werden, wird zunächst der Diskriminator trainiert.
...
Der Discriminator versucht nun die Daten richtig zuzuordnen. Wie erfolgreich dies geschieht, lässt sich mithilfe der MinMax-Verlustfunktion errechnen.
Nun gilt es festzustellen, welche Kantengewichte wie angepasst werden müssen, um den Verlust zu reduzieren.
Dazu werden unter Zuhilfenahme von Backpropagation entsprechende Gradienten berechnet und dann die Gewichte der Kanten in die entgegengesetzte Richtung der Gradienten angepasst (Gradientenabstieg). 
Zu beachten ist, dass während des Trainings des Diskriminators die Gewichte des Generators nicht verändert werden. Dieser Teil des GANs wird in dieser Phase sogar komplett ausgeklammert.

### Zweite Phase
Sind die Gewichte des Diskriminators aktualisiert worden, so widmet sich der nächste Schritt dem Training des Generators.
...
Die Unterscheidung zur ersten Phase liegt unter anderem darin, dass der Diskriminator jetzt nicht sowohl mit echten als auch unechten Daten gefüttert wird, sondern nur mit unechten.
Der Diskriminator versucht nun möglichst alle Daten als unecht zu erkennen. Auch hier wird ein Verlust berechnet, wobei die Funktion eine andere sein kann als beim Training des Diskriminators. 
Der Generator wird bestraft, wenn der Diskriminator ein generiertes Bild als solches erkennt.
Um die Gewichte des Generators anzupassen, muss die Backpropagation des Verlustes folglich über das gesamte GAN erfolgen.
Negative Auswirkungen auf die Leistung des Diskriminators lassen sich unterbinden, indem dessen Gewichte zuvor festgesetzt werden und für diesen Schritt unveränderlich sind.
...
Das Auftreten des Problems lässt sich nicht pauschal vorhersagen, jedoch neigt beispielsweise die MinMax-Verlustfunktion dazu. 
Die Wahl anderer Verlustfunktionen, wie beispielsweise dem <a href="https://en.wikipedia.org/wiki/Wasserstein_metric">Wassersteinverlust</a>, wirken dem Phänomen entgegen [^2]. 

### Instabilität/Oszillation
Das Training von GANs gestaltet sich schwierig, da die beiden Teilnetze gegeneinander arbeiten. Die Eingaben können unzählige Dimensionen haben. Ferner können auch Kostenfunktionen verwendet werden, die nicht konvex sind. [^5]
...
Die Wahl der Hyperparameter kann den Unterschied zwischen einem konvergierenden GAN und einem GAN, welches mit dem Training gar nicht erst beginnt, machen.


### Mode Collapse
Mode Collapse beschreibt einen Zustand in dem der Generator Daten ausgibt, die weniger divers sind als die echten Daten.
Der Generator produziert also beispielsweise nur Bilder von Ananas, obwohl in der Verteilung der echten Daten Ananas, Orangen und Kirschen vorkommen.
...
Das Training des Generators sieht im Detail folgendermaßen aus:
...
Ein letztes hier erwähntes Mittel zur Verhinderung von Mode Collapse ist die mini-batch discrimination. Hierbei wird errechnet, wie ähnlich die Instanzen innerhalb eines Batch sind.
...
Aus dieser Eingabe $z$ wird eine Ausgabe $G(z)$ aus der dem Generator zugrunde liegenden Wahrscheinlichkeitsdichte $p_g$ generiert. $p_g$ hat dieselbe Definitionsmenge wie $p_{data}$.
Es handelt sich beim Diskriminator $D$ um ein binäres Klassifikationsnetzwerk. Das neuronale Netz wird in diesem Fall auf die MinMax-Verlustfunktion optimiert:
...
Um beispeilsweise in Keras `binary_crossentropy` als Verlustfunktion zu nutzen, müssen beim Training des Diskriminators die echten Daten $x$ mit dem Label $y=1$ und die unechten Daten aus $G(z)$ mit dem Label $y=0$ an $D$ übergeben werden.
...
Die Gewichte $\theta_d$ werden nun entsprechend angepasst. Dazu wird der Gradient $\nabla_{\theta_d}$ berechnet. Je nachdem ob $V(D,G)$ oder `binary_crossentropy` verwendet wird, muss ein Gradientenaufstieg oder -abstieg durchgeführt werden.
...
Damit wird sowohl bei der Nutzung von $V(D,G)$ als auch bei der Nutzung der binären Kreuzentropie dasselbe Optimum erreicht.
...
Das Nash-Gleichgewicht ist erreicht, wenn die Verteilung der echten Daten identisch ist mit der Verteilung der generierten Daten, also $p_{data}(x)=p_{g}(x)$ für alle $x$ [^1].
Der Diskriminator klassifiziert dann zu 50% richtig, was einem Münzwurf gleichkommt. 
...
[^1]: Ian J. Goodfellow et al., "Generative Adversarial Nets", https://arxiv.org/pdf/1406.2661.pdf
